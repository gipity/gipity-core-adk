      ...
      try {
        ...
        try {
          await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("Capacitor init timeout")),
                2000,
              ),
            ),
          ]);
          ...
          debug.warn(
            "Capacitor initialization timed out, continuing anyway:",
            error,
          );
        }
        ...
        try {
          ...
          const envInfo = (await Promise.race([
            getEnvironmentInfo(),
            new Promise((resolve) =>
              setTimeout(() => resolve(defaultEnvInfo), 1000),
            ),
          ...
          if (envInfo.isServiceWorkerSupported && envInfo.isNative) {
            try {
              ...
              await Promise.race([
                ...
                new Promise((_, reject) =>
                  setTimeout(() => reject(new Error("SW timeout")), 1500),
                ),
              ]);
      ...
      >
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Initializing {import.meta.env.VITE_APP_NAME || "App"}...
          </p>
        </div>
      </div>
    );
